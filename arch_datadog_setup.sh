...
report() {
    ESC_LOG=$(python2 -c 'import sys, urllib; print urllib.quote(sys.stdin.read().strip())' < "$LOGFILE")
    ESC_OS=$(uname | python2 -c 'import sys, urllib; print urllib.quote(sys.stdin.read().strip())')
    ESC_API_KEY=$(echo "$DD_API_KEY" | python2 -c 'import sys, urllib; print urllib.quote(sys.stdin.read().strip())')
    ESC_AGENT_VERSION=$(echo "$AGENT_VERSION" | python2 -c 'import sys, urllib; print urllib.quote(sys.stdin.read().strip())')
...
# Grab any param from the python2 config.
get_from_py_config()
{
    cd $DD_HOME/agent >/dev/null 2>&1
    local param=$(PYTHONPATH='agent/checks/libs:$PYTHONPATH' $DD_HOME/venv/bin/python2 -c "import config ; print config.$1")
    cd - >/dev/null 2>&1
...
#######################################################################
detect_python2() {
    if command -v python22.7; then
        export PYTHON_CMD="python22.7"
    elif command -v python22; then
        # FreeBSD apparently uses this
        export PYTHON_CMD="python22"
    else
...
print_green "* python2 found, using \`$PYTHON_CMD\`"
...
print_console "* Setting up a python2 virtual env"
...
VENV_PYTHON_CMD="$DD_HOME/venv/bin/python2"
...
if [ "$(uname)" = "SunOS" ]; then
    # Install pyexpat for our version of python2, a dependency for xml parsing (varnish et al.)
    # Tested with /bin/sh
